...
namespace Blazor.Utilities.ReportManager
{
    ...
    public sealed class TestCaseProvider
    {
        ...
        private void AddPicturesOnFail()
        {
            try
            {
                var tcFolder = Path.Combine(GetImagesAndVideoFullPath(),
                    GetCurrentTcName());

                var dir = new DirectoryInfo(tcFolder);
                var images = dir.GetFiles("*.png");

                foreach (var image in images)
                {
                    _currentTestCase.Log(LogStatus.Error,
                        _currentTestCase.AddScreenCapture(image.FullName));
                }
            }
            catch (Exception ex)
            {
        ...
        private void AddScreenCast(string videoPath)
        {
        ...
        public void LogTestCaseFinalStatus(string status, string errorMessage,
            string testContextStackTrace, string tcFullName, string testDescription,
            string videoFullName, string source, string tcName)
        {
            LogStatus logStatus;
            ...
            switch (status)
            {
                case "Failed":
                    ...
                    fullError = TestCaseHasFailed(errorMessage,
                        testContextStackTrace, tcFullName,
                        testDescription, videoFullName,
                        source, tcName, logStatus);
                    break;
                case "Inconclusive":
                    logStatus = LogStatus.Warning;
                    fullError = TestCaseHasFailed(errorMessage,
                        testContextStackTrace, tcFullName,
                        testDescription, videoFullName, source, tcName, logStatus);
                    break;

                case "Skipped":
                    logStatus = LogStatus.Skip;
                    AddPicturesOnFail();
                    LoggerManager.Instance.Warning(@"Test ended with: [ " + logStatus + @" ]. ", true);
                    break;

                case "Passed":
                    logStatus = LogStatus.Pass;
                    LoggerManager.Instance.Information(@"Test ended with: [ " + logStatus + @" ]. ", true);
                    break;
        ...
        private string TestCaseHasFailed(string errorMessage,
           string testContextStackTrace, string tcFullName,
           string testDescription, string videoFullName,
           string source, string tcName, LogStatus finalStatus)
        {
            ...
            //Log to report
            LoggerManager.Instance.Error($"Test ended with: [ {finalStatus} @]. {errorForLog}", true);
        ...
        private void LogFailedTCs(string tcName)
        {
            try
            {
                if (!File.Exists(_fileLogsFailed))
                {
                    ...
                    //Log
                    LoggerManager.Instance.Information($"File to log failed tests: {_fileLogsFailed} created.");
                }
            ...
            {
                //Log
                LoggerManager.Instance.Error(tcName + " - " + ex.Message);
            }
        }
        ...
        private void LogFailedTcsWithErrorMessage(string tcName, string errorMessage)
        {
            try
            {
                if (!File.Exists(_fileLogsFailedAndMessage))
                {
                    ...
                    //Log
                    LoggerManager.Instance.Information($"File to log failed tests and error messages: {_fileLogsFailedAndMessage} created.");
                }
